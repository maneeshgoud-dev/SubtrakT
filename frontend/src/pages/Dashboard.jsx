  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <p className="text-slate-400 text-sm mb-7">All your subscriptions at a glance</p>

        {/* Stat Cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <StatCard label="Total" value={subscriptions.length} sub="subscriptions" />
          <StatCard label="Active" value={active.length} sub="subscriptions" />
          <StatCard label="Due this week" value={upcoming.length} sub="renewals" />
          <StatCard label="Est. monthly" value={`~${totalMonthly.toFixed(2)}`} sub="spend (mixed currencies)" />
        </div>
        ...
        {!loading && !error && subscriptions.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-lg font-semibold text-slate-300 mb-2">No subscriptions yet</p>
            <p className="text-sm text-slate-500">Click <strong className="text-indigo-400">+ Add</strong> to track your first one.</p>
          </div>
        )}
        ...
        {!loading && subscriptions.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {subscriptions.map((sub) => (
              <SubscriptionCard key={sub._id} subscription={sub} onDelete={handleDelete} />
            ))}
          </div>
        )}
        ...
        <div className="mt-16 mb-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-slate-800/40 backdrop-blur border border-slate-700/40 rounded-2xl p-6 hover:border-indigo-500/30 transition-all duration-200">
              ...
              <h4 className="font-semibold text-white mb-2">Add your subscriptions</h4>
              <p className="text-sm text-slate-400 leading-relaxed">
                Click <strong className="text-slate-200">+ Add</strong> and enter your subscription details — name, price,
            ...
            <div className="bg-slate-800/40 backdrop-blur border border-slate-700/40 rounded-2xl p-6 hover:border-amber-500/30 transition-all duration-200">
              ...
              <h4 className="font-semibold text-white mb-2">We track renewals for you</h4>
              <p className="text-sm text-slate-400 leading-relaxed">
            ...
            <div className="bg-slate-800/40 backdrop-blur border border-slate-700/40 rounded-2xl p-6 hover:border-emerald-500/30 transition-all duration-200">
              ...
              <h4 className="font-semibold text-white mb-2">Get reminder emails</h4>
              <p className="text-sm text-slate-400 leading-relaxed">Reminder emails are sent automatically before each renewal:</p>
              <ul className="mt-3 space-y-1.5">
                <li className="flex items-center gap-2 text-xs text-slate-400">
                  <span className="w-2 h-2 rounded-full bg-indigo-400 shrink-0"></span>
                  <span><span className="text-slate-200 font-medium">Monthly / Yearly</span> — 7, 3 and 1 day before</span>
                </li>
                <li className="flex items-center gap-2 text-xs text-slate-400">
                  <span className="w-2 h-2 rounded-full bg-amber-400 shrink-0"></span>
                  <span><span className="text-slate-200 font-medium">Weekly</span> — 3 and 1 day before</span>
                </li>
                <li className="flex items-center gap-2 text-xs text-slate-400">
                  <span className="w-2 h-2 rounded-full bg-emerald-400 shrink-0"></span>
                  <span><span className="text-slate-200 font-medium">Daily</span> — 1 day before</span>
                </li>
              </ul>
              <p className="text-xs text-slate-500 mt-3">Each email includes the subscription name, renewal date and amount due.</p>
            </div>
